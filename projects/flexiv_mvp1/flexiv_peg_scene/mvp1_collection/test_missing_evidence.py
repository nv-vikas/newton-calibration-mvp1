...
def test_absent_evidence_reports_readiness_and_blocks_fit_plan(tmp_path: Path, generic: bool):
    ...
    env = ArticulationEnvCfg(usd_path=str(asset), joint_groups={"arm": ("joint1",)},
                            joint_map={"joint1": "joint1"}) if generic else SO101EnvCfg(usd_path=str(asset))
...
def test_missing_asset_still_blocks_absent_evidence(tmp_path: Path):
    env = ArticulationEnvCfg(usd_path=str(tmp_path / "missing.usd"),
                            joint_groups={"arm": ("j",)}, joint_map={"j": "j"})
